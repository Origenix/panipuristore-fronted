...
import { Download, Smartphone, ShieldCheck, Zap, CheckCircle2, ArrowDownToLine, ExternalLink } from 'lucide-react';

const APK_DOWNLOAD_URL = 'https://github.com/Origenix/panipuristore-fronted/releases/latest/download/PanipuriStore.apk';
                    ...
                    <div className="relative grid lg:grid-cols-[1.05fr_0.95fr] gap-10 lg:gap-16 items-center p-7 md:p-12 lg:p-16">
                        <div>
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-xs font-black uppercase tracking-[0.16em] mb-6">
                                <Smartphone className="w-4 h-4" /> PanipuriStore Android App
                            </div>
                            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[1.02] mb-6">
                                PanipuriStore,<span className="block gradient-text">in your pocket.</span>
                            </h1>
                            <p className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-2xl mb-8">
                                Get the latest PanipuriStore Android app directly from the official website.
                                Download the production APK and install it on your compatible Android device.
                            </p>
                            <div className="flex flex-col sm:flex-row gap-3 mb-9">
                                ...
                                <div className="btn-secondary px-5 py-4 text-sm cursor-default">
                                    <ShieldCheck className="w-5 h-5 text-success" /> Official PanipuriStore build
                                </div>
                            </div>
                            <div className="grid sm:grid-cols-3 gap-3">
                                {[
                                    ['Latest build', 'Downloads the APK attached to the latest GitHub Release'],
                                    ['Standalone', 'Install directly on compatible Android devices'],
                                    ['Verified source', 'Served from the official PanipuriStore repository'],
                                ].map(([title, text]) => (
                                ...
                                <div className="rounded-[1.95rem] bg-[#171717] p-6 md:p-8 text-white overflow-hidden">
                                    <div className="flex items-center justify-between mb-10">
                                        <div className="flex items-center gap-3">
                                            <div className="w-12 h-12 rounded-2xl bg-primary flex items-center justify-center shadow-lg shadow-primary/30"><Smartphone className="w-6 h-6" /></div>
                                            <div><p className="font-black tracking-tight">PANIPURI STORE</p><p className="text-xs text-white/50">Premium Street Food</p></div>
                                        </div>
                                    ...
                                    <div className="rounded-3xl bg-white/5 border border-white/10 p-5 mb-5">
                                        ...
                                        <p className="text-sm text-white/60 leading-relaxed">Download the current production APK with one tap.</p>
                                    </div>
                                    <button onClick={handleDownload} className="w-full rounded-2xl bg-white text-[#171717] py-4 px-5 font-black flex items-center justify-center gap-2 hover:-translate-y-0.5 transition-transform">
                                        <ArrowDownToLine className="w-5 h-5" /> Get the latest APK
                                    </button>
                                    <a href="https://github.com/Origenix/panipuristore-fronted/releases" target="_blank" rel="noopener noreferrer" className="mt-3 w-full rounded-2xl border border-white/10 bg-white/5 text-white/80 py-3 px-5 text-sm font-bold flex items-center justify-center gap-2 hover:bg-white/10 transition-colors">
                                        <ExternalLink className="w-4 h-4" /> View releases
                                    </a>
                                    <p className="text-[11px] text-center text-white/35 mt-4">PanipuriStore official website download</p>
                                </div>
                            </div>
                        </div>
                    </div>
